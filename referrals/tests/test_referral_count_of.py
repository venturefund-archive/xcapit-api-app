...
def test_referral_count_of_value_with_wallet(set_fixtures_referrals_case_1):
    ...
    referrals = DefaultNextLevelReferrals(list(User.objects.filter(id=1).values_list('referral_id', flat=True)))
...
def test_referral_count_of_value_without_wallet(set_fixtures_referrals_case_1):
    ...
    referrals = DefaultNextLevelReferrals(list(first_level.all().referred_id))
